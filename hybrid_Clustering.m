function [ J ] = hybrid_Clustering( data, clusters_n, max_t )
...
% initialization
[idx, C] = kmeans(data, clusters_n, 'Options', statset('MaxIter', max_t));
particles(1,:,:) =  C(:,:);
for i=2:particles_n
...
for t=1:max_t
    ...
    for i=1:particles_n
        for j=1:data_n
           for k=1:clusters_n
              d(i,j,k) = sqrt((data(j,1)-particles(i,k,1))^2+(data(j,2)-particles(i,k,2))^2);%sum((data(j,:)-particles(i,k,:)).^2)); 
           end
           [~, idx] = min(d(i,j,:));
           cluster(i,j) = idx(1);
        end
